...
-   Un modelo jerárquico nos da la flexibilidad de aprender de cada tanque, pero nos ayuda a regularizar en aquellas observaciones sin tanta información, aprendiendo del resto de los tanques.

-   Contraintuitivamente, si analizamos el último modelo, los renacuajos pequeños sobreviven mejor. Esta es una de las conclusiones del paper original.

-   Parece haber un efecto de habituación o de estrategias defensivas efectivas entre los renacuajos. Además, la densidad inicial parece jugar un rol crítico, con densidades más altas posiblemente conduciendo a un menor éxito de supervivencia debido a la competencia intraespecífica o a una mayor visibilidad para los depredadores.
...
```{r}
posterior_samples <- As.mcmc.list(fit3)
mcmc_trace(posterior_samples, pars = c("mu_alpha","sigma_alpha", "alpha_tank[1]"))
```

```{r}
fit3_diagnosticos <- rstan::summary(fit3, 
                               probs = c(0.025,
                                         0.5,
                                         0.975),
                               pars=c('mu_alpha','sigma_alpha'))$summary 
...
rhats <- summary_fit3$summary[, "Rhat"]
names(rhats) <- rownames(summary_fit3$summary)

color_scheme_set("brightblue")

mcmc_rhat(rhats)+ ggtitle("R hat para todos los parámetros del modelo")

```

Observamos que las cadenas se ven bien mezcladas, y esto a su vez se refleja en un Rhat muy cercana a 1 para todos los parametros y un buen numero effectivo de muestras.

#### Chequeos a priori

Para poner una inicial necesitamos conocimiento de dominio, sabemos que al estar hablando de un experimento controlado, diferente a una situación natural, los tanques no tienen más de 35 *reedfrogs*. Será extremo obtener una tasa de supervivencia menor al 3% (1 de los 35) y sabemos que puede existir valores por encima de 97% (34 de 35) sin embargo también se espera que no sean los menos.

Proponemos nuestro modelo de optimización "validación cruzada" para encontrar los hiperapriori

```{r}
set.seed(987)
p1_values <- seq(-3, 3, length.out = 100)  # media de mu
p2_values <- seq(0.1, 5, length.out = 100)  # desv est de mu
p3_values <- seq(0.1, 5, length.out = 100)  # parám de sigma_alpha
...
print(paste0("Parámetros de Miu: norm(",
             round(par1,2),
             ",",
             round(par2,2),")"))

print(paste0("Parámetro de sigma: exp("
             ,round(par3,2)
             ,")"))
```
